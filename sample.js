...
process:function(set, preset, completion) {
	...
	async.waterfall([
		...
		function(callback) {
			...
			preset.outputPathFinal = strings.endsWithRemove(preset.outputPath, path.sep);
			preset.outputPathFinal = path.join(preset.outputPath, path.basename(preset.absBasePath));
